...
public class Kernel
{
    ...
    // Synchronized Queues
    private static SyncQueue waitQueue;  // for threads to wait for their child for WAIT && EXIT part 1
    private static SyncQueue ioQueue;    // I/O queue for RAWREAD, RAWWRITE, && SYNC part 2
	...
	TCB myTcb; 	 // needed for *this thread id 
	int cur_tid = 0; // needed for sleep condition && 2 param wakeup
	int cur_pid = 0; // needed to wake thread waiting on *this condition
	switch( irq ) {
	case INTERRUPT_SOFTWARE: // System calls
	    ...
	    case WAIT:
			...
			cur_tid = myTcb.getTid();
			return waitQueue.enqueueAndSleep(cur_tid);
	    ...
	    case EXIT:
			...
			cur_tid = myTcb.getTid(); // calling thread id
			cur_pid = myTcb.getPid(); // condition
			// This function can receive the calling thread's
			// ID, (tid) as the 2nd argument. This tid will be passed to the thread
			// that has been woken up from enqueueAndSleep.
			waitQueue.dequeueAndWakeup(cur_pid, cur_tid);
		...
		return OK;
	    case RAWREAD: // read a block of data from disk ~~ ¯\_(ツ)_/¯
